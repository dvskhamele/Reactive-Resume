...
import { simpleLocalStorageService } from "./simple-local-storage";
...
export const mockAxiosInstance: AxiosInstance = {
  ...
  get: async <T = any>(url: string, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    try {
      ...
      if (url === "/user/me") {
        const user = simpleLocalStorageService.getUser();
        return createMockResponse<T>(user as unknown as T);
      }
      
      if (url === "/resume") {
        const resumes = simpleLocalStorageService.getResumes();
        return createMockResponse<T>(resumes as unknown as T);
      }
      
      if (url.startsWith("/resume/")) {
        const id = url.split("/")[2];
        const resume = simpleLocalStorageService.getResumeById(id);
        if (resume) {
  ...
  post: async <T = any>(url: string, data?: any, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    try {
      ...
      if (url === "/auth/login") {
        const result = simpleLocalStorageService.login(data.email, data.password);
        return createMockResponse<T>(result as unknown as T);
      }
      
      if (url === "/auth/register") {
        const result = simpleLocalStorageService.register(data.email, data.password, data.name);
        return createMockResponse<T>(result as unknown as T);
      }
      
      if (url === "/auth/logout") {
        const result = simpleLocalStorageService.logout();
        return createMockResponse<T>(result as unknown as T);
      }
      
      // Handle resume creation
      if (url === "/resume") {
        const result = simpleLocalStorageService.createResume(data?.name);
        return createMockResponse<T>(result as unknown as T);
      }
  ...
  patch: async <T = any>(url: string, data?: any, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    try {
      // Handle user updates
      if (url === "/user/me") {
        const result = simpleLocalStorageService.updateUser(data);
        return createMockResponse<T>(result as unknown as T);
      }
      ...
      if (url.startsWith("/resume/")) {
        const id = url.split("/")[2];
        const result = simpleLocalStorageService.updateResume(id, data);
        return createMockResponse<T>(result as unknown as T);
      }
  ...
  delete: async <T = any>(url: string, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    try {
      ...
      if (url.startsWith("/resume/")) {
        const id = url.split("/")[2];
        const result = simpleLocalStorageService.deleteResume(id);
        return createMockResponse<T>(result as unknown as T);
      }
  ...
  head: async <T = any>(url: string, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    return createMockResponse<T>({} as T);
  },
  
  options: async <T = any>(url: string, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    return createMockResponse<T>({} as T);
  }
};